...
class CasesApi extends Controller 
{
	...
	function perfect() {
		...
		try {
			...
			$query
				->select('emp.id', 'emp.nik', 'emp.name')
				->tableAlias('tempTablePerfect', $perfect_employee)
				->joinLeft('employee emp', 'emp.id', 'tempTablePerfect.id');
	...
	function notapin() {
		...
		try {
			...
			// no tap in employee
			//-----------------------------------------------------
			$no_tap_in_employee = function($query) use($vars, $queries) {

				$query
					->select('id')
					->from('employee')
					->where('`status`', 1)
					->whereIn('id', $queries['null_arrive_employee']);

			};

			$query
				->select('emp.id', 'emp.nik', 'emp.name')
				->tableAlias('tempTableNotapin', $no_tap_in_employee)
				->joinLeft('employee emp', 'emp.id', 'tempTableNotapin.id');
	...
	function absent() {
		...
		try {
			...
			// absent employee
			//-----------------------------------------------------
			$absent_employee = function($query) use($vars, $queries) {

				$query
					->select('id')
					->from('employee')
					->where('`status`', 1)
					->whereIn('id', $queries['absent_employee']);
			...
			$query
				...
				->joinLeft('employee emp', 'emp.id', 'tempTableAbsent.id');
