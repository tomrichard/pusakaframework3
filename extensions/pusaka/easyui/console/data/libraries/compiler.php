<?php 
namespace Pusaka\Easyui\Library;
...
class Compiler {
	...
	function componentCompiler( $lines ) {
		...
		if( preg_match('/@easyui\:(.+)/', $lines[0], $match) > 0 ) {
			$name = trim($match[1]);
		}
		...
		*/
		...
		$component = file_get_contents( __DIR__ . '/../template/components.tpl.php' );

		$component = strtr($component, [
			'{component_url}'	=> strtr($name, ['.' => '/']),
			'{function_name}' 	=> $name,
			'{uniqe}' 			=> "_".md5($name),//strtoupper(uniqid()),
			'{param}'			=> implode(',', $vars),
			'{value}'			=> implode(',', $value),
			'{function_body}'	=> $body
		]);
	...
	function compile() {
		...
		foreach ($lines as $line) {
			...
			if( preg_match('/@endeasyui;/', $line) > 0 ) {
